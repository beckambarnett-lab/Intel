...
export class HorizonBloom {
   ...
   */
  update(fire: FireView | null, eye: Vec2 | null, screen: { width: number; height: number }): void {
    if (!fire || !eye || fire.dead) {
      this.sprite.visible = false;
      return;
    }
    ...
    // Close enough that the fire is on screen and lighting things itself.
    if (distM <= BLOOM_MIN_RANGE_M || distM >= BLOOM_MAX_RANGE_M) {
      this.sprite.visible = false;
      return;
    }

    // 0 at the near edge, 1 at the far edge.
    const t = (distM - BLOOM_MIN_RANGE_M) / (BLOOM_MAX_RANGE_M - BLOOM_MIN_RANGE_M);
    ...
    this.sprite.alpha = BLOOM_MAX_ALPHA * brightness * (1 - t) * (1 - t);
